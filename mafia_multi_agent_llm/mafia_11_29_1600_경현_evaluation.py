    ...
    def run_prompt(self, prompt, summarize=True, twice=True):
        ...
        while True:
            ...
            if twice:
                ...
                feedback_response = self.client.chat.completions.create(model=model, temperature=1, messages=full_prompt, max_completion_tokens=self.max_tokens) ### 11/28 경현
...
class Game:
    ...
    def first_day(self):
        ...
        for player in self.players:
            if player.user:
                ...
                statement=player.run_prompt({'role':'system','content':text + 'Do not disclose your role to anyone when you introduce yourself.'}) ### 11/28 경현 - 역할 공개 금지 충고 문구 추가
    ...
    def day(self):##11/13 준하
        ...
        for _ in range(10):
            #if self.players[0].alive and (not talk_player.user) and (not next_talk_player.user) and input("Will you talk?(y/n): ")=='y':  ### 11/29 민경현 - 주석 처리
            #    next_talk_player=self.players[0] ### 11/29 민경현 - 주석 처리
