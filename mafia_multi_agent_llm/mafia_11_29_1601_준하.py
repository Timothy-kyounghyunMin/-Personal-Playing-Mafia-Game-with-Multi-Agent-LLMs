    ...
    def run_prompt(self, prompt, summarize=True, twice=True):
        ...
        while True:
            ...
            if twice:
                ...
                full_prompt.append({'role':'user', 'content':feedback_response.choices[0].message.content}) ### 11/28 경현
        ...
        with open(self.file_name, 'a') as json_file:
            ...
            json_file.write('\n')
            if twice:
                json.dump(f'{self.player_name}\'s Feedback: {feedback_response.choices[0].message.content}', json_file)## 11/29 준하
                json_file.write('\n')
    ...
    def night_mafia(self,day):
        ...
        if len(mafia_players) == 1:
            if mafia_players[0].user:
                ...
                print("-------- test ---------")  ### 11/28 민경현
                print(f"Mafia {mafia_players[0].player_name} picked {target_name}") ### 11/28 민경현
                print("-------- test ---------")  ### 11/28 민경현
        ...
        for player in mafia_players:
            if player.user:
                ...
                print("-------- test ---------")  ### 11/28 민경현
                print(f"Mafia {player.player_name} picked {response}") ### 11/28 민경현
                print("-------- test ---------")  ### 11/28 민경현
                candidate.append(response)
        ...
        for _ in range(10):
            ...
            if not user_is_mafia: ### 11/28 민경현
                print("-------- test ---------")  ### 11/28 민경현
                print(f"{mafia_players[i].player_name}: {statement}") ### 11/28 민경현
                print("-------- test ---------")  ### 11/28 민경현
        ...
        for player in mafia_players:
            if player.user:
                ...
                print("players_alive: ",players_alive)
                while True:
                    response = player.run_prompt({'role':'system','content':text},twice=False)
            ...
            if player.user:
                ...
                while True:
                    ...
                    if target_name in players_alive:
                        break
                print("-------- test ---------")  ### 11/28 민경현
                print(f"Doctor {player.player_name} picked {target_name}") ### 11/28 민경현
                print("-------- test ---------")  ### 11/28 민경현
            player.append_memory('system',text)
            ...
            if player.user:
                ...
                while True:
                    ...
                    if target_name in players_alive:
                        break
                print("-------- test ---------")  ### 11/28 민경현
                print(f"Police {player.player_name} picked {target_name}") ### 11/28 민경현
                print("-------- test ---------")  ### 11/28 민경현
            player.append_memory('system',text)
            player.append_memory('assistant',target_name)
    ...
    def day(self):##11/13 준하
        ...
        for _ in range(10):
            ...
            if talk_player.user:
                ...
                #talk_player.append_memory(talk_player.player_name+": "+statement)
                self.append_players_memory(talk_player.player_name,statement,self.players_summarizers)
                next_player_name=talk_player.run_prompt({'role':'system','content':'Choose the next player to talk to except you. One word, no speaker label'}, summarize=False, twice=False)
