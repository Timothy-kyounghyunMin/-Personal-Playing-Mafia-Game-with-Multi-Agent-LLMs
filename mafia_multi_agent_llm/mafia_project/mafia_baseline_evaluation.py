...
class Player:
    ...
    """
    def __init__(self, client, max_tokens, player_name, player_number, use_gpt4, user=False): ## 11/11 경현 other_players 삭제 ## 11/12 준하 수정
        self.client=client
    ...
    def run_prompt(self, prompt):
        ...
        full_prompt=full_prompt+self.memory+[{'role':'system','content':str(self.final_prompt)}]
...
class Game:
    ...
    def __init__(self, openai_api_key, max_tokens, player_count, discussion_depth, use_gpt4, render_markdown, num_mafia=2, num_citizen=4, doctor=True, police=True):
        ...
        #self.players.append(Player(self.client, self.max_tokens, input("Insert your name : "), 0, False, self.file_name, user=True))##11/12 준하 ### 11/29 경현 - 주석 처리
        #agent_names=self.get_player_names(self.player_count-1, [self.players[0].player_name]) ### 11/29 경현 - 주석 처리
        agent_names=self.get_player_names(self.player_count) ### 11/29 경현
        for i in range(len(agent_names)):
            self.players.append(Player(self.client, self.max_tokens, agent_names[i],i+1,self.use_gpt4))
    ...
    def play(self):
        ...
        # 1. 밤부터 시작            
            ...
            self.append_players_memory('system','Night'+str(day)+' start',self.players)
            #   플레이어 순서대로 행동
            ...
            self.append_players_memory('system','Day'+str(day)+' start',self.players)
        ...
        for i, accuracy in enumerate(self.accuracy): ### 11/29 민경현
            print(f'Accuracy day{i+2}: {accuracy}') ### 11/29 민경현
        np.save(f'accuracy/baseline_accuracy_{time.strftime("%Y%m%d_%H%M%S")}.npy', self.accuracy)
        ...
        file_name = f"record_baseline_{timestamp}.json"
    ...
    def initialize_game(self):
        ...
        self.mafia_id=[]
        self.append_players_memory('system','Players are'+str(self.player_names),self.players)
    ...
    def introduce_players(self):
        ...
        for player in self.players:
            ...
            if player.role == 'mafia':
                ...
                self.append_players_memory('system',text,[player])
                if player.user: 
                    print('System:'+text)
    ...
    def first_day(self):
        ...
        self.append_players_memory('system',text,self.players)
        for player in self.players:
            if player.user:
                print('System: '+text)
                statement=input(player.player_name+': ')
            else:
                statement=player.run_prompt(None)
                print(player.player_name+': '+statement)
            self.append_players_memory(player.player_name,statement,self.players)
    ...
    def night_mafia(self,day):
        ...
        self.append_players_memory("system", "Two mafia select other target. Debate with other mafia about selecting target. If debate ends, please remain silent until instructed by the system.",mafia_players)
        ...
        for _ in range(5):
            ...
            self.append_players_memory(mafia_players[i].player_name,statement,mafia_players)
            i=1-i
        ...
        if candidate[0] == candidate[1]: ## 2차 투표 결과가 같은 경우
            ...
            #self.players[self.player_names.index(target_name)].alive = False
            for player in mafia_players:
                player.final_prompt['MAFIA_SUMMARY'].append(f'TNight{day} target is {target_name}.')
            return target_name
    ...
    def night_doctor(self,day):##11/12 준하
        ...
        if len(doctor_players)==1:
            ...
            self.append_players_memory('system',text,doctor_players)
            self.append_players_memory(player.player_name,target_name,doctor_players)
    ...
    def night_police(self,day):##11/12 준하
        ...
        if len(police_players)==1:
            ...
            if player.user:
                ...
                target_name = player.run_prompt({'role':'system','content':text})
            self.append_players_memory('system',text,police_players)
            self.append_players_memory(player.player_name,target_name,police_players)
            ...
            if player.user:
                print(output_text)
            self.append_players_memory('system',output_text,police_players)
    ...
    def check_dead(self, mafia_target, doctor_save,day):
        if mafia_target==doctor_save:
            ...
            text=f'The Mafia tried to kill someone, but the doctor saved him/her. No one died last night!'
        else:
    ...
    def day(self):##11/13 준하
        ...
        self.append_players_memory('system', 'It is now daytime. Time for discussion.',players_alive)
        talk_player = players_alive[random.randint(int(self.players[0].alive),len(players_alive)-1)]
        next_talk_player = talk_player
        for _ in range(10):
            #if self.players[0].alive and (not talk_player.user) and (not next_talk_player.user) and input("Will you talk?(y/n): ")=='y': ### 11/29 민경현
            #    next_talk_player=self.players[0] ### 11/29 민경현
            ...
            if talk_player.user:
                ...
                self.append_players_memory(talk_player.player_name,statement,self.players)
                ...
                #talk_player.append_memory(talk_player.player_name+": "+statement)
                self.append_players_memory(talk_player.player_name,statement,self.players)
                next_player_name=talk_player.run_prompt({'role':'system','content':'Choose the next player to talk to except you. One word, no speaker label'})
    ...
    def vote(self,day):##11/13 준하
        ...
        while True:
            ...
            self.append_players_memory('system','Vote result is'+str(votes),players_alive)
            if len(max_voters)==1:
                break
            print("System: There are multiple candidates with the highest votes, so we will proceed with a revote.")
            self.append_players_memory('system', 'There are multiple candidates with the highest votes, so we will proceed with a revote.',players_alive)
        suspect=self.players[self.player_names.index(max_voters[0])]
        self.accuracy.append(correct_count/citizen_count) ### 11/29 민경현
        #최후의 변론
        print("System: The suspect will now have the opportunity for a final statement.")
        self.append_players_memory('system','The suspect will now have the opportunity for a final statement.',players_alive)
        statement=suspect.run_prompt({'role':'system','content':'You are suspect! Make your final statement.'})
        self.append_players_memory(suspect.player_name,statement,players_alive)
        print(statement)
        #최종투표
        print('System: It\'s time for final vote!')
        self.append_players_memory('system', 'It\'s time for final vote!',players_alive)
        final_vote={'y':[],'n':[]}
        yes_count=0
        for player in players_alive:
            if player==suspect:
        ...
        self.append_players_memory('system', final_vote_result, players_alive)
        for player in self.players:
