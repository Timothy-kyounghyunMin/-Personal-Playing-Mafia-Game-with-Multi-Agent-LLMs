...
class Player:
    ...
    def run_prompt(self, prompt, summarize=True, twice=True):
        model = 'gpt-4o-mini'
        if self.user :
        ...
        while True:
            ...
            if twice:
                ...
                #feedback_text = f'Review the utterance you just provided to determine whether it aligns with your role, complies with the game rules, and is a well-reasoned response. Please create a sentence with fewer than {str(self.max_tokens)} tokens.' ### 11/28 경현 
                #11/29 준하
                feedback_text = f'''Here is statement you generated: {response1}
                Now, evaluate your recent statement based on the following questions(within 40 tokens each.):
                1. Did your statement contribute to the progression of the dialogue? (e.g., by adding new information, perspectives, or suspicions)
                2. Did you respond to or build on another player’s statement? (e.g., was there interaction rather than an independent comment?)
                3. Did your statement create tension or express suspicion? (e.g., did you name someone suspicious or defend your actions?)
                4. Was your statement aligned with your character role? (e.g., Mafia avoiding suspicion, Citizen fostering teamwork)
                5. Did you propose any actionable outcomes? (e.g., pointing out suspicions, suggesting votes, or asking questions)'''
                full_prompt.append({'role':'system', 'content':f'Your name is {self.player_name}, your role is {self.role}. {feedback_text}'}) ### 11/28 경현
                feedback_response = self.client.chat.completions.create(model=model, temperature=1, messages=full_prompt, max_completion_tokens=self.max_tokens*2) ### 11/28 경현
                full_prompt.append({'role':'assistant', 'content':feedback_response.choices[0].message.content}) ### 11/28 경현 ### 11/30 준하 role 변경
                #contemplation = f'If there are any aspects that need improvement, revise the response and answer again. Please create a sentence with fewer than {str(self.max_tokens)} tokens.' ### 11/28 경현
                contemplation = f'''Modify your first answer by reffering to the feedback.
                First answer:{response1}
                Feedback:{feedback_response.choices[0].message.content}''' ##11/29 준하
...
class Game:
    ...
    def __init__(self, openai_api_key, max_tokens, player_count, discussion_depth, use_gpt4, render_markdown, num_mafia=2, num_citizen=4, doctor=True, police=True, output=None):
        self.client=OpenAI(api_key=openai_api_key)
        self.max_tokens=max_tokens
        ...
        agent_names=self.get_player_names(self.player_count)
    ...
    def play(self):
        ...
        # 1. 밤부터 시작            
            ...
            if end:
                print(end_text)
                self.append_players_memory('system',end_text,[])
                break
            self.day() # 토론
        ...
        for player in self.players:  ### 11/28 민경현
            print(f"{player.player_name}: {player.role}")  ### 11/28 민경현
        for i, accuracy in enumerate(self.accuracy):
            print(f'Accuracy day{i+2}: {accuracy}')
        np.save(f'accuracy/accuracy_{time.strftime("%Y%m%d_%H%M%S")}.npy', self.accuracy)
    ...
    def initialize_game(self):
        ...
        for i in range(self.player_count):
            ...
            self.append_players_memory('system',f'{self.players[i].player_name}: {role_list[i]}',[])
    ...
    def first_day(self):
        ...
        for player in self.players:
            if player.user:
                ...
                statement=player.run_prompt({'role':'system','content':text + 'Do not disclose your role to anyone when you introduce yourself.'},twice=False) ### 11/28 경현 - 역할 공개 금지 충고 문구 추가 ##11/29 준하 - twice 변경
    ...
    def night_mafia(self,day):
        ...
        if len(mafia_players) == 1:
            if mafia_players[0].user:
                ...
                target_name = mafia_players[0].run_prompt({'role':'system', 'content':text},twice=False)
                #rendered_target_name = return_dict_from_json_or_fix(self.client, self.max_tokens, target_name, self.use_gpt4)
                mafia_players[0].append_memory('system',text)
                mafia_players[0].append_memory('assistant',target_name)
                self.append_players_memory('system',text,[])
                self.append_players_memory(mafia_players[0].player_name,target_name,[])
        ...
        for player in mafia_players:
            if player.user:
                ...
                response = player.run_prompt({'role':'system', 'content':text},twice=False)
                candidate.append(response)
        ...
        for player in mafia_players:
            if player.user:
                ...
                #response = player.run_prompt({'role':'system', 'content':text},twice=False) ### 11/28 민경현 - 원래 코드
                ### 11/28 민경현 - 잘못된 사람을 지목하는 경우 방지
                players_alive=[player.player_name for player in self.players if player.alive]
                while True:
                    response = player.run_prompt({'role':'system','content':text},twice=False)
                    if response in players_alive:
                        break
    ...
    def night_doctor(self,day):##11/12 준하
        ...
        if len(doctor_players)==1:
            ...
            if player.user:
                ...
                #target_name = player.run_prompt({'role':'system','content':text},twice=False) ### 11/28 민경현 - 원래 코드
                ### 11/28 민경현 - 잘못된 사람을 지목하는 경우 방지
                players_alive=[player.player_name for player in self.players if player.alive]
                while True:
                    target_name = player.run_prompt({'role':'system','content':text},twice=False)
                    if target_name in players_alive:
                        break
            player.append_memory('system',text)
            player.append_memory('assistant',target_name)
            self.append_players_memory('system',text,[])
            self.append_players_memory(player.player_name,target_name,[])
    ...
    def night_police(self,day):##11/12 준하
        ...
        if len(police_players)==1:
            ...
            self.append_players_memory('system',output_text,[])
    ...
    def check_dead(self, mafia_target, doctor_save,day):
        if mafia_target==doctor_save:
            #text=f'Mafia shot {mafia_target}, but doctor save {mafia_target}!'  ### 11/28 민경현 - 주석 처리
            text=f'The Mafia tried to kill someone, but the doctor saved him/her. No one died last night!' ### 11/28 민경현
        else:
    ...
    def day(self):##11/13 준하
        ...
        for _ in range(10):
            ...
            if talk_player.user:
                ...
                #talk_player.append_memory(talk_player.player_name+": "+statement)
                self.append_players_memory(talk_player.player_name,statement,self.players_summarizers)
                next_player_name=talk_player.run_prompt({'role':'system','content':'Choose the next player to talk to except you. One word, no speaker label. It\'s better to suspect or talk to a particular person'}, summarize=False, twice=False)
    ...
    def vote(self,day):##11/13 준하
        ...
        mafia_players = [player.player_name for player in self.players if player.role == 'mafia' and player.alive] ### 11/29 민경현
        citizen_count = 0 ### 11/29 민경현
        correct_count = 0 ### 11/29 민경현
        while True:
            ...
            for player in players_alive:
                if player.user:
                    ...
                    pointed=player.run_prompt({'role':'system','content':'Say only suspect\'s name. No speaker label'}, twice=False)
                    if player.role != 'mafia': ### 11/29 민경현
                        citizen_count += 1 ### 11/29 민경현
                        if pointed in mafia_players: ### 11/29 민경현
                            correct_count += 1 ### 11/29 민경현
        ...
        self.accuracy.append(correct_count/citizen_count)
        #최후의 변론
        ...
        for player in players_alive:
            ...
            if player.user:
                ...
                response=player.run_prompt({'role':'system','content':'Kill?(y/n)'}, twice=False)
    ...
    def append_players_memory(self, name, memory_item, players):#한 번에 메모리 추가
        role=None
        content=memory_item
        self.history.append(f'{name} : {content}')    
